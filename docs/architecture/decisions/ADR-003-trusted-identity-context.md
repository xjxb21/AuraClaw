...
- 前置决策：[ADR-001](./ADR-001-production-service-boundaries.md)、[ADR-002](./ADR-002-hands-boundary-and-connectors.md)
...
- 部门 ID 在创建 Root Session 时从已验签 Assertion 固化，并透传到 MCP Header/`_meta`；
- Tool arguments 中同名 tenant/user/dept 只可作为兼容数据；冲突则 fail closed；
...
阶段门禁见 [开发阶段校验清单](../../development/stage-gates.md) 的 I44（身份归属）与 I45（部门快照透传）。
