...
### M12g：价格洞察 Java 原子 Tool 接入

- [x] 保留现有 Skill、11 个 ToolCapability 名称与输入/输出契约，底层执行器可配置切换 Java。
- [x] development combined server 与生产 action-hands 使用同一 Java Executor 装配规则。
- [x] AgentSession claim/resolve 使用至少 32 字符的共享 workload Token，scope 由 Java 路由绑定。
- [x] CT-TOOL-REQUEST-V2 哈希与 Java 共享字面向量一致，每次调用签发一次性 Assertion。
- [x] Java 业务 Tool Header 只含 Assertion、Invocation ID 与 Content-Type。
- [x] handoff code、页面 access token、workload Token 和 Assertion 不进入事件或 Runtime 状态。
- [x] Java camelCase DTO 字段转换为现有 snake_case Tool 输出契约。
- [x] `uv.lock` 仅保留 RFC 8785 JCS 依赖，已移除不再使用的 PyJWT。
- [x] Java 授权/Tool 目标测试 18 passed；冒烟脚本、M3 安全和 M12 本地调试相关回归
  33 passed、1 deselected，跳过项为 Windows 环境不存在 `/usr/bin/env` 的既有用例。
- [x] 触碰文件 Ruff、8 个核心源码 Mypy 与 10 条 import-linter contract 通过。
- [ ] 在具备 Java AgentSession、Redis、两端共享 workload Token 和价格 DWD 的联调环境执行真实 HTTP E2E。
