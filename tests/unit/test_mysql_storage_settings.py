...
def test_update_returning_followup_drops_assigned_claim_predicates() -> None:
    from auraclaw.infrastructure.persistence.mysql_pool import (
        _update_returning_followup_sql,
    )

    sql = _update_returning_followup_sql(_CLAIM_FINALIZE_SQL)
    assert sql is not None
    normalized = " ".join(sql.split()).lower()
    assert normalized.startswith("select * from artifact.metadata where")
    assert "tenant_id=$1" in normalized
    assert "artifact_id=$2" in normalized
    assert "upload_id=$3" in normalized
    assert "finalize_claim_expires_at" not in normalized
    assert "status in" not in normalized


def test_mysql_fetchrow_update_returning_reads_row_after_claim() -> None:
    """UPDATE RETURNING must not re-apply SET-invalidated claim predicates."""
    import asyncio
    ...
    async def exercise() -> None:
        ...
        row = await connection.fetchrow(
            _CLAIM_FINALIZE_SQL, "t1", "art_1", "up_1", "tok", 30
        )
        assert row is not None
        assert row["status"] == "scanning"
        assert raw.claimed is True
        assert any(sql.lstrip().upper().startswith("UPDATE") for sql in raw.statements)
        assert any(sql.lstrip().upper().startswith("SELECT") for sql in raw.statements)
