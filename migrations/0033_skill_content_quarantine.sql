...
ALTER TABLE hands.skill_admission_audit
    ...
    CHECK (outcome IN ('accepted','rejected','quarantined'));
ALTER TABLE hands.skill_admission_audit
    ADD CONSTRAINT skill_admission_audit_result_check
    CHECK (
        (outcome = 'accepted' AND stage = 'completed' AND safe_error_code IS NULL)
        OR (
            outcome IN ('rejected','quarantined')
...
CREATE INDEX skill_admission_audit_failure_idx
    ...
    WHERE outcome IN ('rejected','quarantined');
